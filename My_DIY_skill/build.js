...
const OUTPUT = path.join(BASE, 'index.html');
...
function scan(dir, relative = '') {
    ...
    for (const item of fs.readdirSync(dir).sort()) {
        ...
        } else if (/\.(md|txt|html)$/i.test(item) && item !== 'index.html' && item !== 'viewer.js' && item !== 'build.js') {
...
console.log(`Generated index.html (${(fs.statSync(OUTPUT).size / 1024).toFixed(0)} KB)`);
